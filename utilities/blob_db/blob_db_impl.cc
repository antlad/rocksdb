...
void EvictAllVersionsFilter::Callback(int level, const Slice& key,
  ...
  if (!is_new && value_type == CompactionFilter::kValue) {
    ...
    bool succ =
        BlobDBImpl::parseLSMValue(existing_value, &file_number, &handle);
    ...
    if (succ) {
      if (impl_->debug_level_ >= 3)
        Log(InfoLogLevel::INFO_LEVEL, impl_->db_options_.info_log,
            ...
            key.ToString().c_str(), sn, is_new, file_number, handle.offset(),
            handle.size());

      impl_->override_vals_q_.enqueue(
          {file_number, key.size(), handle.offset(), handle.size(), sn});
    }
  } else {
    if (impl_->debug_level_ >= 3)
      Log(InfoLogLevel::INFO_LEVEL, impl_->db_options_.info_log,
  ...
  if (handle.offset() <
    ...
    if (debug_level_ >= 2) {
      Log(InfoLogLevel::ERROR_LEVEL, db_options_.info_log,
          ...
          file_number, handle.offset(), handle.size(), key.data());
    }
...
bool BlobDBImpl::markBlobDeleted(const Slice& key, const Slice& lsmValue) {
  ...
  succ = findFileAndEvictABlob(file_number, key.size(), handle.offset(),
                               handle.size());
  return succ;
}
...
void BlobFile::setFromFooter_locked(const BlobLogFooter& footer) {
  BlobLogHeader hdr;

  if (footer.HasTTL()) header_.setTTLGuess(footer.GetTTLRange());

  if (footer.HasTimestamp()) header_.setTSGuess(footer.GetTimeRange());
