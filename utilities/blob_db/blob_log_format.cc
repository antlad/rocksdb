...
BlobLogHeader::BlobLogHeader() : magic_number_(kMagicNumber) {}
...
Status BlobLogFooter::DecodeFrom(Slice* input) {
  ...
  tsrange_t temp_ts;
  if (!GetFixed64(input, &temp_ts.first) || !GetFixed64(input, &temp_ts.second))
...
Status BlobLogHeader::DecodeFrom(Slice* input) {
  ...
  if (has_ttl) setTTLGuess(temp_ttl);

  tsrange_t temp_ts;
  if (!GetFixed64(input, &temp_ts.first) || !GetFixed64(input, &temp_ts.second))
    return Status::Corruption("Invalid Blob Log Header: timestamp");
  if (has_ts) setTSGuess(temp_ts);
...
void BlobLogRecord::resizeKeyBuffer(size_t kbs) {
  if (kbs > kbs_) {
    key_buffer_.reset(new char[kbs]);
    kbs_ = kbs;
  }
}
...
void BlobLogRecord::resizeBlobBuffer(size_t bbs) {
  if (bbs > bbs_) {
    blob_buffer_.reset(new char[bbs]);
    bbs_ = bbs;
  }
}
